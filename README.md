# IDEA with Segmented Crystal Electromagnetic Precision Calorimeter (SCEPCal)
...
## SCEPCal Features
- Fully automatic and adjustable geometry
- Supports different front/rear crystal tower divisions
- Quasi-projective main segmented layer with adjustable projective offsets
- Pure-projective single-layer timing
...
```
If you want to compile your own copy:
...
```sh
ddsim --steeringFile example/SteeringFile_IDEA_o2_v01.py
```
or, using command line options,
```sh
ddsim --steeringFile example/SteeringFile_IDEA_o2_v01.py -G --gun.direction "1 1 0" --gun.energy "1*GeV" --gun.particle="gamma" -O gamma_1GeV.root
```

### Crystal Geometry

Detector dimensions and options are defined in the `<define>` tag in `FCCee/IDEA/compact/IDEA_o2_v01/SCEPCal_o1_v01.xml`. The interesting parameters are the crystal widths and front/rear tower divisions. The tower width (theta width) refers to the nominal square width of a single crystal tower, which will then contain the specified number of NxN crystals for the front and rear compartments. Care should be taken to make sure these numbers are consistent with what is intended. A tower width of 1cm with 1F/1R divisions will make the default 1:1 configuration of 1x1cm crystals for both front/rear. A tower width of 3cm with a 3F/2R division will make a 3x3 array of 1x1cm front crystals, and a 2x2 array of 1.5x1.5cm rear crystals, etc. (All dimensions nominal).
...
Set all options in the steering file `example/SteeringFile_IDEA_o2_v01.py`. Parameters can also be set via the command line at runtime.  Refer to dd4hep documentation for more details.

#### Event selection

Input MC files can be set in the steering file above, or the built-in ddsim particle gun can be used. If both are enabled, both will run. `wzp6_ee_ZZ_test_ecm240_1k.stdhep` with 1k events is provided as an example here:

```python
SIM.inputFiles = ['/eos/user/w/wochung/wzp6_ee_ZZ_test_ecm240_1k.stdhep']
```
...
Activate key4hep to enable edm4hep in ROOT:
```
source /cvmfs/sw.hsf.org/key4hep/setup.sh
```

Utilities to read the ROOT collections into python objects are provided in the standalone SCEPCal repo: https://github.com/wonyongc/SCEPCal/blob/main/python/scepcal.py. Use that file or copy the definitions as you wish.
